...
RE_LICENCE_CLIENT = re.compile('licence_client\.CLI[0-9]{8}\.crt')
...
class SSLCrypt:
    """
    See: https://www.section.io/engineering-education/rsa-encryption-and-decryption-in-python/
    e.g.:
    fd = open('/etc/kastmenu/keys/kwad/access.dat', 'rb')
    c = fd.read()
    fd.close()

    s = SSLCrypt.decode('/etc/kastmenu/keys/kwad/kwad.client.key', c)
    c = SSLCrypt.encode('/etc/kastmenu/keys/kwad/kwad.client.rsa.pub', s)
    """
...
def checkCertificateValidity(peercert=None, rootcert=None, intermediate_certs=None):
    """ See https://stackoverflow.com/questions/30700348/how-to-validate-verify-an-x509-certificate-chain-of-trust-in-python
    - peercert, rootcert are pem as:
        a) String, or
        b) Bytes file or
        c) absolute file paths.
    - intermediate_certs is a list of intermediate certificates pem as:
        a) String, or
        b) Bytes file or
        c) absolute file paths.
    """
    ...
    # Test Only:
    xrootcert = b("""-----BEGIN CERTIFICATE-----
MIICWsdfgsdfhgdghfchjgjkhBBjliuomimipomiDTYHRYUYTJKfgdgdtgfdHGHg
-----END CERTIFICATE-----
    """)
    xintermediate_certs = [b("""-----BEGIN CERTIFICATE-----
MIICWsdfgsdfhgdghfchjgjkhBBjliuomimipomiukDDDDDdgfdfgdgdtgfdHGHg
...
-----END CERTIFICATE-----
    """)]
    xpeercert = b("""-----BEGIN CERTIFICATE-----
MIICWsdfgsdfhgdghfchjgjkhkjliuomimipomiukjTThgbdgfdfgdgdtgfdHGHg
...
-----END CERTIFICATE-----
    """)
    ...
    # -----------------------
    # Check is Path ?
    if isinstance(rootcert, str) and  rootcert.find('/')>0:
        if not path.isfile(rootcert):raise
        with open(rootcert, 'rb') as f:rootcert = f.read()
    if not isinstance(rootcert, (bytes, bytearray)):rootcert = bytes(rootcert, 'utf-8')
    rootcert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, rootcert)
    ...
    # Set Validation Time:
    ## validation_date = datetime.strptime("2022-10-01", "%Y-%m-%d")
    ## store.set_time(validation_date)

    for intermediate_cert in intermediate_certs:
        # Check is Path ?
        if isinstance(intermediate_cert, str) and   intermediate_cert.find('/') > 0:
            if not path.isfile(intermediate_cert): raise
            with open(intermediate_cert, 'rb') as f:
                intermediate_cert = f.read()
        if not isinstance(intermediate_cert, (bytes, bytearray)): intermediate_cert = bytes(intermediate_cert, 'utf-8')
        intermediate_cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, intermediate_cert)
    ...
    # Check is Path ?
    if isinstance(peercert, str) and  peercert.find('/')>0:
        if not path.isfile(peercert):raise
        with open(peercert, 'rb') as f:peercert = f.read()

    if not isinstance(peercert, (bytes, bytearray)):peercert = bytes(peercert, 'utf-8')

    peercert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, peercert)
    store_ctx = OpenSSL.crypto.X509StoreContext(store, peercert)
    try:
        store_ctx.verify_certificate()
    except OpenSSL.crypto.X509StoreContextError as e:
        raise
...
def storePassword(user, password):
    self_funct='storePassword'
    EXCEPT_USERS = ('kwad', 'ksvn', 'ksvnsa')
    if user in EXCEPT_USERS:raise
    sanitize(user)
    sanitize(password)
    from kwadlib.tools import getTempDir
    from kwadlib.tools import Popen, SUBPROCESS_STDOUT, SUBPROCESS_PIPE

    pubfile = getKastConfs()['local_client_pub']
    temp_dir = getTempDir()

    cmd = 'echo {password} > {temp_dir}/p.txt && openssl rsautl -encrypt -inkey {pubfile} -pubin -in {temp_dir}/p.txt -out /etc/kastmenu-{konf_install_type}/keys/kwad/{user}-access.dat && rm {temp_dir}/p.txt '.format(password=password, pubfile=pubfile, temp_dir=temp_dir, konf_install_type=getKastConfs()['install_type'], user=user)
    #1:
    p = Popen(cmd, stdout=SUBPROCESS_PIPE, stderr=SUBPROCESS_STDOUT, universal_newlines=True, shell=True)
    stdout, stderr = p.communicate()
    ret = p.wait()
    stdout_stderr = stdout
    if ret!=0:raise Exception('Class: Main, Method: %s: Unable to securely store password for user: %s' % (self_funct, user))
...
def expectSSlKey(cmd, passfile, keyfile, message="assword:", twice=False, verbose=0):
    ...
    if not passfile in P_CACHES:
        ...
        fd.close()
        P_CACHES[passfile] = c
        # P_CACHES[passfile] = SimpleCrypt.encode('key', check_output(['openssl', 'rsautl', '-in', passfile, '-decrypt', '-inkey', keyfile]).decode("utf-8").strip())
        # openssl   rsautl -in /etc/kastmenu/conf/keys/kwad/access.dat -decrypt   -inkey   /etc/kastmenu/conf/keys/kwad/kwad.client.key
    ...
    if verbose >= 10:
        print ('<-Class: Main, Method: %s Success running command: %s.' % (selfMethod, cmd))
    return data
...
def sshNoPassword(host, user, password, type='rsa', ssh_port=22, verbose=0):
    ...
    if cuser != default.KAST_USER:
        raise Exception(self_funct + ': is allowed only to user: %s. Not for user: %s !' % (default.KAST_USER, cuser))
...
EOF"
    ...
    # Copy to RemoteHost:
    # -------------------
    # sudo -u kastserver cat /home/kastmenu/.ssh/id_rsa.pub
    ...
    try:
        # 1:
        child = pexpect.spawn(cmd, encoding='utf-8', timeout=3, echo=False, logfile=None)
...
def sha256(value):
    ...
    # if isinstance(value, bytes): return sha256(value).hexdigest()
    # sha256(value.encode('utf-8')).hexdigest()


def encryptLocal(value):
    ctx = '492755ef3fb4ef68ecb7f6b5b288daaa8aa8492755ef3fb4ef68ecb7f6b5b288daaa8a90' # can be anything
    v = sha256(ctx)
    # Key:
    AES256_KEY = bytes(v[:32], 'ascii')  # 32 digits
    AES256_IV = bytes(v[len(v) - 16:], 'ascii')  # 16 digits

    return AES256Crypt.encrypt(padTo16(value.strip()), key=AES256_KEY, iv=AES256_IV)[0]
def decryptLocal(xvalue):
    ctx = '492755ef3fb4ef68ecb7f6b5b288daaa8aa8492755ef3fb4ef68ecb7f6b5b288daaa8a90' # can be anything
    v = sha256(ctx)
    # Key:
    AES256_KEY = bytes(v[:32], 'ascii')  # 32 digits
    ...
    return AES256Crypt.decrypt(xvalue, key=AES256_KEY, iv=AES256_IV)
