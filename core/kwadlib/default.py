...
KAST_DBPATH='/var/kastmenu/db/kastmenu.db'
KASTSERVER_LOG='kastserver.log'
KAST_CONF_DIR = '/etc/kastmenu'
KAST_CONF = '/etc/kastmenu/kast.conf'
KAST_DESC_CONF = '/etc/kastmenu/descs/kast.desc.conf'


KAST_USER="kastserver"
...
def getTempDir():
    ...
    if getUser() == KAST_USER:
        TEMP_DIR = path.normpath(getKastConfs()['temp_dir'])
    else:
...
def containerdInit():
    ...
    if not path.isdir("/var/kastmenu/db"):
        """
        Deduced /var/kastmenu as been mounted by CTR Management (e.g.: docker)
        and the apt postinst var config was overwrite by clean mounted dir.
        e.g.: nerdctl vs docker mout parameter:
            --mount src="/<myrealhost>/var/kastmenu",target=/var/kastmenu,type=bind  
            --mount src="/<myrealhost>/var/log/kastmenu",target=/var/log/kastmenu
        So we recreate:
        """
        mkdir("/var/kastmenu/db")
        mkdir("/var/kastmenu/langs")
        mkdir("/var/kastmenu/tmp")



KASTSERVER_STDOUT = '/var/log/kastmenu/kastserver.log'
...
def  getKastAgentBinPath():
    self_funct = 'getKastAgentBinPath'
    from os import path
    from kwadlib import xception

    kpath = getInstallDir() + '/kastagent/kastagent-' + getVersion() + '.tar.gz'

    if not path.isfile(kpath):
        raise xception.kwadSystemException('Main', self_funct, 'Unable to find Kastagent binary file at: %s !' % kpath)
...
def  getVersion():
    ...
    return path.split(d)[1].split('kastmenu-')[-1].split('.tar.gz')[0]