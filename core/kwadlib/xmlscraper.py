...
"""
File: scraper.py.
This file is inspired from scraper.py.
The author of scraper.py is: Michael Foord
This file has been adapted for AskApy, Kikonf then KastMenu.
This is the original header of scraper.py :
...
class Scraper:
    ...
    def tagstart(self):
        ...
        for entry in matchlist:
            ...
            if self.isCapSensitive():attrs.append((attrname, attrvalue))
            else:attrs.append((attrname.lower(), attrvalue))
            
        if self.isCapSensitive():return self.handletag(name, attrs, thetag)              # deal with what we've found.
        else:return self.handletag(name.lower(), attrs, thetag)              # deal with what we've found.
...
#    a = approxScraper('http://www.pythonware.com/daily', 'approx.py')
    a = Scraper()
    a.feed(open("/somewhere/test.html").read())                   # read and feed
    #open('/tmp/index2.html','w').write(a.close())
