...
def printlog(fromf, message, request=None, level=0, verbose=0):
    if request!=None:more = ' %s %s' % (request.uri, request.remote_ip)
    else:more=''
    if verbose >= level:print (str(datetime.now()) + '%s [%s] ' % (more, fromf) + message)
