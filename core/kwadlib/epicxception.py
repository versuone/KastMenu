...
class BaseException(Exception):
    def __init__(self, fromClass, fromMethod, message, code=None, prefix=None):
            ...
            self.__value = self.prefix + 'From class:' + str(self.__fromClass) + ', from method:' + str(self.fromMethod) + ' ' + str(self.__message)
...
class BaseInformation:
    def __init__(self, fromClass, fromMethod, message, code=None, prefix=None):
            ...
            self.__value = self.prefix + 'From class:' + str(self.__fromClass) + ', from method:' + str(self.fromMethod) + ' ' + self.__message
    ...
    def warn(self, verbose=0):
        ...
        else:message=self.prefix + self.__class__.__name__ + ': ' + 'From class:' + str(self.__fromClass) + ', from method:' + str(self.fromMethod) + ' ' + self.__message
...
## Project: epicxml
class epicxmlBaseException(BaseException):
    def __init__(self, fromClass, fromMethod, message, code=None, prefix=None):
        if prefix==None:prefix='EEpicxml'
        BaseException.__init__(self, fromClass, fromMethod, message, code=code, prefix=prefix)

class epicxmlBaseInformation(BaseInformation):
    def __init__(self, fromClass, fromMethod, message, code=None, prefix=None):
        if prefix==None:prefix='IEpicxml'
...
## Project: epicxml
class epicxmlParameterTypeException(epicxmlBaseException):
    def __init__(self, fromClass, fromMethod, parm, expected, received, code=None, prefix=None):
        self.parm=parm
        self.expected=expected
        self.received=received
        epicxmlBaseException.__init__(self, fromClass, fromMethod, 'Incorrect type for ' + str(parm) + ', expected type:' + str(expected) + ', received:' + str(received) + '.', code=code, prefix=prefix)
    ...
    def __reduce__(self):
        return ( epicxmlParameterTypeException, (self.fromClass, self.fromMethod, self.parm, self.expected, self.received, self.code, self.prefix) )
    
class epicxmlParameterException(epicxmlBaseException):pass
class epicxmlSystemException(epicxmlBaseException):pass
class epicxmlAttrNotFound(epicxmlBaseException):pass
class AppPreexistingObjectError(epicxmlBaseException):pass
class AppNonPreexistingObjectError(epicxmlBaseException):pass
class AppComponentIntegrityError(epicxmlBaseException):pass
class epicxmlXmlSyntaxException(epicxmlBaseException):pass
class epicxmlPxQueryException(epicxmlBaseException):pass
class epicxmlPxQueryNoNodeFoundException(epicxmlBaseException):pass
class epicxmlUnSupportedTagException(epicxmlBaseException):pass
class epicxmlUnSupportedTagAttributeException(epicxmlBaseException):pass
class epicxmlDescriptorCheckException(epicxmlBaseException):pass
class epicxmlRestrictorCheckException(epicxmlBaseException):pass
class epicxmlOptionException(epicxmlBaseException):
    def __init__(self, fromClass, fromMethod, code=None, prefix=None):
        epicxmlBaseException.__init__(self, fromClass, fromMethod, 'Not supported option !', code=code, prefix=prefix)
        
    # python bug (>2.4<=2.6.5), only concern Exception classes: (c)pickle is unable to call subclass with a different amount of args from moether Exception class !
    def __reduce__(self):
        return ( epicxmlOptionException, (self.fromClass, self.fromMethod, self.code, self.prefix) )

class epicxmlInformation(epicxmlBaseInformation):pass