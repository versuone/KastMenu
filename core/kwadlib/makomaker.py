...
class  MakoMaker:
    ...
    def operate(self):
        ...
        from mako import exceptions as mako_exceptions # A003 Mako verbosity

        try:
            result = Template(self.__sourceIn, strict_undefined=True, 
                cache_dir=self.__tempDir, module_directory=self.__tempDir, 
            lookup = TemplateLookup(directories=self.__template_lookup),).render(**self.__vars)
                
        except Exception as e: # A003:
            print(mako_exceptions.text_error_template().render())
            if self.__fileOut!=None:fo=' on file:' + self.__fileOut
            else:fo=''
            raise tmplxception.tmplmkSystemException(str(self.__class__), selfMethod, 'Mako Template processing error' + fo + ' ! SubException is: ' + str(e) + '.')
        except: # A003:
            print(mako_exceptions.text_error_template().render())
            if self.__fileOut!=None:fo=' on file:' + self.__fileOut
            else:fo=''
            raise tmplxception.tmplmkSystemException(str(self.__class__), selfMethod, 'Mako Template processing error' + fo + ' !')
